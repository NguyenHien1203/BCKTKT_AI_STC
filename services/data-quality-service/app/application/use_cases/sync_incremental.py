...
class IncrementalSyncService:
    ...
    def run_sync(
        ...
        # `source_format`/`schema_fields` được đính kèm (denormalized) để
        # UC-029 không phải gọi đồng bộ ngược lại ingestion-service chỉ để
        # lấy lược đồ dataset — raw_object_key ở đây luôn là tệp JSON do
        # `_storage.upload()` phía trên ghi ra.
        if records:
